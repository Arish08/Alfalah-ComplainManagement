...
// STEP 3: creates an empty E-Form for each new Human Task
...
export default function WorkflowDesignerPage({ workflowId, isNew, navigate }) {
        ...
        const mappedNodes = sourceNodes.map((n) => {
          ...
          return {
            ...
            data: {
              ...
              // STEP 3:
              // Load the E-Form saved with this workflow step
              eForm: config.eForm || {
                id: null,
                name: '',
                description: '',
  ...
  // STEP 3:
  // Every Human Task gets its own empty E-Form.
  const addHumanNode = () => {
  ...
  const deleteSelection = () => {
    if (selectedNodeId) {
      setNodes((current) =>
        current.filter((x) => x.id !== selectedNodeId)
      )
      ...
      setEdges((current) =>
        current.filter((x) => x.id !== selectedEdgeId)
      )
  ...
  const toPayload = () => ({
    ...
    nodes: nodes.map((node) => ({
      ...
      // STEP 3:
      // Save the E-Form inside the existing ConfigJson.
      configJson:
        Number(node.data.nodeType) === 1
          ? JSON.stringify({
              assignmentMode:
  ...
  const save = async () => {
    ...
    try {
      ...
      const saved = isNew
        ? await api('/workflow-definitions', {
            method: 'POST',
            body: payload,
          })
      ...
      if (isNew) {
        navigate(`/workflows/${saved.id}/designer`)
      }
            ...
            <div className="inspector-form">
              ...
              <label className="field compact">
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateSelectedNode({
                      nodeType: Number(e.target.value),
                    })
                  }
                ...
                <>
                  ...
                  <label className="field compact">
                    ...
                    <input
                      ...
                      value={
                        selectedNode.data.escalationRoleCode || ''
                      }
                  ...
                  <label className="field compact">
                    ...
                    <select
                      ...
                      value={
                        selectedNode.data.assignmentMode || 'queue'
                      }
            ...
            <div className="inspector-form">
              ...
              <label className="field compact">
                ...
                <input
                  ...
                  value={
                    selectedEdge.data?.outcomeKey || ''
                  }

                  onChange={(e) =>
                    updateSelectedEdge({
                      outcomeKey: e.target.value,
                    })
                  }
